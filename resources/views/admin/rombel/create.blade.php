...
@section('title', 'Tambah Rombel | SISMIK')
    ...
    <div class="main-content">
        ...
        <div class="content-header">
            ...
            <div class="header-text">
                ...
                <p>Tambah data kelas baru ke sistem</p>
            </div>
        </div>
            ...
            <form action="{{ route('admin.rombel.store') }}" method="POST">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="tahun_pelajaran" class="form-select" required>
                            ...
                            @foreach($tahunList as $tahun)
                                <option value="{{ $tahun }}" 
                                    {{ old('tahun_pelajaran', $periodeAktif->tahun_pelajaran ?? '') == $tahun ? 'selected' : '' }}>
                                    {{ $tahun }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="semester" class="form-select" required>
                            ...
                            <option value="Ganjil" {{ old('semester', $periodeAktif->semester ?? '') == 'Ganjil' ? 'selected' : '' }}>Ganjil</option>
                            <option value="Genap" {{ old('semester', $periodeAktif->semester ?? '') == 'Genap' ? 'selected' : '' }}>Genap</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_rombel" class="form-control" 
                            value="{{ old('nama_rombel') }}" placeholder="Contoh: X.1, XI IPA 1, XII IPS 2" required>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="wali_kelas" class="form-select" required>
                        ...
                        @foreach($guruList as $guru)
                            <option value="{{ $guru->nama }}" {{ old('wali_kelas') == $guru->nama ? 'selected' : '' }}>
                                {{ $guru->nama }} - {{ $guru->nip }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="latitude" class="form-control" 
                            value="{{ old('latitude') }}" placeholder="Contoh: -7.3041">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Longitude (Opsional)</label>
                        <input type="text" name="longitude" class="form-control" 
                            value="{{ old('longitude') }}" placeholder="Contoh: 110.4267">
                    </div>
                </div>
                ...
                <div class="btn-group-action">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Simpan Rombel
                    </button>
                </div>
            </form>
        </div>
