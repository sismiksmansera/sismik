                    ...
                    <tbody>
                        @forelse($siswaList as $index => $siswa)
                            @php
                                ...
                                $waliAktif = $waliData[$semesterAktifSiswa] ?? '';
                            @endphp
                            <tr data-id="{{ $siswa->id }}" 
                                ...
                                <!-- Guru Wali (Clickable) -->
                                <td class="text-center">
                                    <div class="editable-cell wali-cell" onclick="openWaliModal(this.closest('tr'))">
                                        @if($waliAktif)
                                            <span class="main-value">{{ $waliAktif }}</span>
                                            <span class="sub-info">Smt {{ $semesterAktifSiswa }}</span>
                                        @else
                                            <span class="empty-value">Belum diatur</span>
                                            <span class="sub-info">Smt {{ $semesterAktifSiswa }}</span>
...
<!-- Modal Edit Guru Wali All Semesters -->
<div class="modal" id="modalWali">
    <div class="modal-dialog" style="max-width: 550px;">
        <div class="modal-header" style="background: linear-gradient(135deg, #f59e0b, #d97706);">
            <h3><i class="fas fa-chalkboard-teacher"></i> Edit Wali Kelas Semua Semester</h3>
            <button class="modal-close" onclick="closeModal('modalWali')">&times;</button>
        </div>
        <div class="modal-body">
            <div class="student-info-card">
                <div class="student-info-row">
                    <div class="info-item">
                        <span class="info-label">Siswa</span>
                        <span class="info-value" id="waliSiswaName">-</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">Angkatan</span>
                        <span class="info-value" id="waliAngkatan">-</span>
                    </div>
                </div>
            </div>
            
            <div style="margin-bottom: 12px; font-weight: 600; color: var(--gray-600);">
                <i class="fas fa-chalkboard-teacher"></i> Wali Kelas per Semester
            </div>
            <div class="semester-forms-grid" id="waliFormsGrid">
                <!-- Generated by JS -->
            </div>
            
            <div class="warning-notice" style="background: #fef3c7; border-color: #f59e0b;">
                <i class="fas fa-info-circle" style="color: #f59e0b;"></i>
                <div class="text" style="color: #92400e;">
                    <strong>Semester aktif:</strong> {{ $tahunAktif }} - {{ ucfirst($semesterAktif) }}
                </div>
            </div>
        </div>
        <div class="modal-footer">
            <button class="btn btn-secondary" onclick="closeModal('modalWali')">Batal</button>
            <button class="btn btn-warning" id="saveWali" style="background: linear-gradient(135deg, #f59e0b, #d97706); border: none;">
                <i class="fas fa-save"></i> Simpan Semua
            </button>
        </div>
    </div>
...
const waliOptions = @json($guruWaliList);
...
// Open Wali Modal
function openWaliModal(row) {
    currentRow = row;
    const data = row.dataset;
    const waliData = JSON.parse(data.wali || '{}');
    const semesterAktif = parseInt(data.semesterAktif) || 1;
    
    document.getElementById('waliSiswaName').textContent = data.nama;
    document.getElementById('waliAngkatan').textContent = data.angkatan || '-';
    
    let html = '';
    for (let i = 1; i <= 6; i++) {
        const isActive = i === semesterAktif;
        html += `
            <div class="semester-form-item ${isActive ? 'active' : ''}" style="${isActive ? 'background: #fef3c7; border-color: #f59e0b;' : ''}">
                <div class="semester-header">
                    <span class="semester-label">Semester ${i}</span>
                    ${isActive ? '<span class="semester-badge" style="background: #f59e0b;">Aktif</span>' : ''}
                </div>
                <select class="semester-select" data-semester="${i}">
                    <option value="">-- Pilih Wali Kelas --</option>
                    ${waliOptions.map(w => `<option value="${w}" ${waliData[i] === w ? 'selected' : ''}>${w}</option>`).join('')}
                </select>
            </div>
        `;
    }
    document.getElementById('waliFormsGrid').innerHTML = html;
    
    openModal('modalWali');
}

// Save Wali All
document.getElementById('saveWali')?.addEventListener('click', async function() {
    if (!currentRow) return;
    const nisn = currentRow.dataset.nisn;
    const selects = document.querySelectorAll('#waliFormsGrid .semester-select');
    
    this.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Menyimpan...';
    this.disabled = true;
    
    for (const sel of selects) {
        await fetch('{{ route("admin.siswa.update-wali") }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken },
            body: JSON.stringify({ nisn, semester: sel.dataset.semester, nama_guru_wali: sel.value })
        });
    }
    
    this.innerHTML = '<i class="fas fa-save"></i> Simpan Semua';
    this.disabled = false;
    showToast('Wali Kelas berhasil diperbarui');
    closeModal('modalWali');
    location.reload();
});
