...
@section('title', 'Edit Ekstrakurikuler | SISMIK')
...
<div class="layout">
    ...
    <div class="main-content">
            ...
            <div class="header-content">
                ...
                <div class="header-text">
                    ...
                    <div class="header-subtitle">
                        <i class="fas fa-calendar-alt"></i> Periode: <strong>{{ $ekstra->tahun_pelajaran }} - {{ $ekstra->semester }}</strong>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('guru_bk.koordinator-ekstra.update', $ekstra->id) }}" method="POST" class="ekstra-form" id="ekstraForm">
            @csrf
            @method('PUT')
            ...
            <div class="form-card">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="nama_ekstrakurikuler" name="nama_ekstrakurikuler" 
                           ...
                           value="{{ old('nama_ekstrakurikuler', $ekstra->nama_ekstrakurikuler) }}">
                ...
                <div class="form-group">
                    <label for="tahun_semester">Tahun & Semester</label>
                    <input type="text" id="tahun_semester" class="form-control" 
                           value="{{ $ekstra->tahun_pelajaran }} - {{ $ekstra->semester }}" disabled>
                    <small style="color: #6b7280;">Tahun dan semester tidak dapat diubah</small>
                </div>
                    ...
                    <div class="form-row">
                        <div class="form-col">
                            ...
                            <select id="pembina_1" name="pembina_1" class="form-select">
                                ...
                                @foreach($pembinaList as $pembina)
                                    <option value="{{ $pembina }}" {{ $ekstra->pembina_1 == $pembina ? 'selected' : '' }}>
                                        {{ $pembina }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-col">
                            ...
                            <select id="pembina_2" name="pembina_2" class="form-select">
                                <option value="">- Pilih Pembina -</option>
                                @foreach($pembinaList as $pembina)
                                    <option value="{{ $pembina }}" {{ $ekstra->pembina_2 == $pembina ? 'selected' : '' }}>
                                        {{ $pembina }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-col">
                            ...
                            <select id="pembina_3" name="pembina_3" class="form-select">
                                <option value="">- Pilih Pembina -</option>
                                @foreach($pembinaList as $pembina)
                                    <option value="{{ $pembina }}" {{ $ekstra->pembina_3 == $pembina ? 'selected' : '' }}>
                                        {{ $pembina }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="form-group">
                    ...
                    <textarea id="deskripsi" name="deskripsi" class="form-control" rows="3"
                              placeholder="Deskripsi singkat tentang ekstrakurikuler">{{ old('deskripsi', $ekstra->deskripsi) }}</textarea>
                </div>
            </div>
            ...
            <div class="form-card">
                ...
                <div class="anggota-controls">
                    ...
                    <span id="anggotaCount" class="badge">{{ count($anggotaTerdaftar) }} Anggota</span>
                </div>
                
                <div id="anggotaList" class="anggota-list">
                    @if(count($anggotaTerdaftar) > 0)
                        <div class="loading" id="loadingAnggota">
                ...
                <div id="hiddenAnggotaInputs">
                    @foreach($anggotaTerdaftar as $siswaId)
                        <input type="hidden" name="anggota_ids[]" value="{{ $siswaId }}">
                    @endforeach
                </div>
            </div>
            ...
            <div class="form-actions">
                <a href="{{ route('guru_bk.koordinator-ekstra.index') }}" class="btn btn-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let selectedSiswa = @json(array_map('strval', $anggotaTerdaftar));
    let siswaData = {};
    
    // Load existing anggota on page load
    if (selectedSiswa.length > 0) {
        loadAnggotaTerdaftar();
    }
    
    function loadAnggotaTerdaftar() {
        fetch('{{ route("guru_bk.koordinator-ekstra.get-siswa-by-ids") }}?ids=' + selectedSiswa.join(','))
            .then(res => res.json())
            .then(data => {
                const loadingEl = document.getElementById('loadingAnggota');
    ...
    function loadSiswaData() {
        ...
        formData.append('tahun_aktif', '{{ $ekstra->tahun_pelajaran }}');
        formData.append('semester_aktif', '{{ $ekstra->semester }}');
        
        fetch('{{ route("guru_bk.koordinator-ekstra.get-siswa") }}', {
            method: 'POST',
            body: formData,
            headers: {
    ...
    window.toggleSiswa = function(id) {
        ...
        document.querySelectorAll('.siswa-item').forEach(item => {
            ...
            if (match) {
                item.classList.toggle('selected', selectedSiswa.includes(match[1]));
            }
        });
